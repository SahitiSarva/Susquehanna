...
class DecisionMaker:
    ...
    def calculatePreferences(self):
        ...
        if self.conflict.useManualPreferenceRanking:
            #print("entered using manual preferences")
            self.conflict.options.set_indexes()
            # gets triggered by every decision maker so self = decision maker
            self.payoffs = gmcrUtil.mapPrefRank2Payoffs(
                self.preferenceRanking, self.conflict.feasibles, self.conflict.feasibles.name, self.options, self.name, self.conflict.actual_payoffs)
    ...
    def calculatePerceived(self):
        ...
        #print(len(self.conflict.feasibles), "feasibles")
        #TODO: Fix
        self.perceived = self.conflict.feasibles#FeasibleList(percDash, 2, 5,toOrdered=toOrd)
       # print(self.perceived.yn)
...
class Condition:
    ...
    def __init__(self, conflict, condition):
        ...
        #print("triggering name setting which causes setting indices")
        self.name = self.ynd()
...
class FeasibleList:
    ...
    def __init__(self, dash=None, nr_players = 1, nr_decisions = 1, toOrdered=None, payoffs=None, baseline = None):
        ...
        #feasible_patterns 
        #print(temp)
        #temp = sorted([(int(yn,2), yn) for yn
         #               in gmcrUtil.define_moves(nr_decisions,nr_players)])
        # SS: Made a change her
        ...
        self.name = [gmcrUtil.find_name(yn, nr_decisions, payoffs, baseline) for dec, yn in temp]
...
class ConflictModel:
    ...
    def __init__(self, payoffs, baseline):
        ...
        self.useManualPreferenceRanking = True
    ...
    def recalculateFeasibleStates(self, init_override=False):
        ...
        feasDash = [len(self.decisionMakers)*'Y' + 'N'*(len(self.options)- len(self.decisionMakers))]
        #print(feasDash)
        #feasDash = ['-' * len(self.options)]
        #print(feasDash, "feasDash")
        # for infeas in self.infeasibles:
        #     #print(infeas, "infeasible states as is")
        #     #print(infeas.ynd(), "infeasible conditions in ynd")
        #     res = gmcrUtil.rmvSt(feasDash, infeas.ynd())
        #     feasDash = res[0]
        #     print(feasDash)
        #     infeas.statesRemoved = res[1]
        nr_decisions= int(len(self.options)/len(self.decisionMakers))
        self.feasibles = FeasibleList(feasDash, nr_players = len(self.decisionMakers), nr_decisions = nr_decisions, payoffs = self.actual_payoffs, baseline = self.baseline )

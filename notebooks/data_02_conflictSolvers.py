...
class RMGenerator:
    ...
    def __init__(self, conflict, useCoalitions=False):
        ...
        for dm in self.effectiveDMs:
            ...
            for val in focalCOmoves:
                manipulatedStates = [y + z for y in manipulatedStates
                                     for z in [0, val]]
            #print(manipulatedStates, "manipulated states")
            ...
            for state in fixedStates:
                ...
                #print(reachable, "reachable before removing infeasibles")
                # remove infeasibles
                reachable = [y for y in reachable
                             if (y in conflict.feasibles.decimal)]
                #print( "from state",state)
                #print(dm.name, "for dm")
                #print(reachable, "reachable")

                # add one set of mutually reachable states
                # SS: removed self.conflict.feasibles.toOrdered[state0]-1 replaced it with self.conflict.feasibles.toOrdered[state0]
...
class LogicalSolver(RMGenerator):
    ...
    def checkSanctions(self, focalDM, otherDMs, state0, state1,
        ...
        for dm in otherDMs:
            ...
            for state2 in moves:
                ...
                #print(state0, state2)
                #SS: making this a strict inequality
                #print("checking for strict sanction")
    ...
    def gmr(self, dm, state0):
        ...
        if not ui:
            narration = ("{0} is Nash stable and thus GMR stable for DM {1} since they have no UIs "
                         "from this state.\n").format(
                self.chattyHelper(dm, state0), dm.name)
                # changing this condition as this returns a nash equilibrium
            return True, narration
...
class InverseSolver(RMGenerator):
    ...
    def nashCond(self):
        ...
        for dmIdx, dm in enumerate(self.conflict.decisionMakers):
            #print(self.conflict.feasibles.ordered)
            print(self.desEq, "desired equilibrium")
                ...
                for state1p, state1d in enumerate(self.mustBeLowerNash[dm]):
                    ...
                    if pay0 < pay1:   # if there is a UI available
                        ...
                        for pay2 in (payoffs[dm][state2] for state2 in self.mustBeLowerGMR[dm][state1p]):
                            #print("checks if the sanctioned state is better")
                            #if pay0 == pay2
